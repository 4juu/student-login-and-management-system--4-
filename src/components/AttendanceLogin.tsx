...
export const AttendanceLogin: React.FC<AttendanceLoginProps> = ({
  ...
  const handleCodeInput = (digit: string) => {
    if (code.length < 4) {
      ...
      // Auto-submit when code is 3 or 4 digits
      if (newCode.length >= 3) {
  ...
  const checkAndSubmit = (codeToCheck: string) => {
    if (!activeSessionId) {
      setMessage({ type: 'error', text: 'لا يوجد سجل نشط! الرجاء تفعيل سجل أولاً' });
      setCode('');
      return;
    }
    ...
    if (student) {
      ...
      setMessage({ type: 'success', text: `مرحباً ${student.name}! تم تسجيل حضورك بنجاح` });
      setCode('');
    } else {
      setMessage({ type: 'error', text: 'الرمز غير صحيح. حاول مرة أخرى' });
      setCode('');
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-8">
      ...
      <div className="mt-6 text-center text-gray-600">
        <p>أدخل رمزك المكون من 3 أو 4 أرقام</p>
      </div>
    </div>
  );
};
