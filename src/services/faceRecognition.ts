...
export const extractFaceDescriptorMultiCapture = async (
  ...
  onProgress?.(100);
  console.log(`✅ دُمجت ${descriptors.length} بصمة`);
...
// === سجل التعرف الداخلي ===
const recognitionLog: any[] = [];

export const getRecognitionLog = () => [...recognitionLog];
export const clearRecognitionLog = () => { recognitionLog.length = 0; };

function logMatch(data: any) {
  recognitionLog.unshift({
    ...data,
    time: new Date().toISOString(),
  });
  if (recognitionLog.length > 100) recognitionLog.pop();
}
...
export const findBestMatch = <T extends { faceDescriptor?: number[] | string }>(
  ...
  for (const item of items) {
    ...
    const confidence = Math.round((1 - distance / threshold) * 100);

    logMatch({
      studentName: (item as any).name,
