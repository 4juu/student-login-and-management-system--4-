...
import { loadBlazeFace, detectFacesBlaze } from './blazeFace';
import { loadMobileFaceNet, extractEmbedding, cosineSimilarity } from './mobileFaceNet';
...
const loadModelsInternal = async (): Promise<void> => {
  ...
  try {
    ...
    _emitProgress({ stage: 'detector', stageIndex: 0, percent: 0, detail: '⚠ فشل تحميل كاشف الوجه', error: e.message || 'Unknown error' });
  }

  // Stage 2: MobileFaceNet recognition
...
// ── Face landmarks drawing (no-op — no landmarks in BlazeFace) ──
...
export const drawFaceLandmarks = (
  ...
  // No-op: BlazeFace does not provide 68-point landmarks
};
...
const toFloat32 = (input: any): Float32Array => {
  ...
  if (Array.isArray(input)) {
    ...
    if (input.length > 0 && input.length < DESC_DIM) {
      const looksCompressed =
        input.length % 2 === 0 &&
        Number.isInteger(input[0]) && input[0] >= 0 && input[0] < DESC_DIM &&
        input.length >= 4 &&
        Number.isInteger(input[2]) && input[2] >= 0 && input[2] < DESC_DIM;
      if (looksCompressed) {
        return new Float32Array(ensureDecompressed(input));
      }
    }
...
export const compareMultiDescriptor = (query: Float32Array, stored: MultiDescriptor): number => {
  ...
  for (let i = 0; i < angleCount; i++) {
    ...
    if (sim > bestAngleSim) bestAngleSim = sim;
  }

  const mainDist = 1 - mainSim;
  const angleDist = 1 - bestAngleSim;
  return Math.min(mainDist, mainDist * 0.6 + angleDist * 0.4);
};
...
>(
  ...
  if (!w) {
    for (const s of storedSimple) {
      const sim = cosineSimilarity(query, new Float32Array(s.desc));
      const d = 1 - sim;
...
export const extractFaceDescriptor = async (
  ...
  for (const det of detections) {
    if (det.score > bestScore) {
      bestScore = det.score;
      const emb = await extractEmbedding(input, det.box, vw, vh);
      best = emb;
    }
  }
  return best;
...
export const extractAllFaceDescriptors = async (
  ...
  for (const det of detections) {
    const emb = await extractEmbedding(input, det.box, vw, vh);
    if (emb) {
      results.push({ detection: det, descriptor: emb });
    }
  }
  return results;
};
...
export const detectSingleFace = async (
  ...
  if (!modelsLoaded) await loadFaceModels();
  const detections = await detectFacesBlaze(input, false);
  return detections.length > 0 ? { detection: detections[0] } : null;
};
...
export const compareFaces = (
  ...
  if (a.length !== b.length) {
    ...
    return 1 - cosineSimilarity(normalizeDescriptor(paddedA), normalizeDescriptor(paddedB));
  }
  return 1 - cosineSimilarity(normalizeDescriptor(a), normalizeDescriptor(b));
};
